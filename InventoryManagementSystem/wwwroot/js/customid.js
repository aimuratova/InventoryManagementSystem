$(document).ready(function () {
    ...
    function addRow() {
        ...
        <div class="row align-items-center mb-2 elementRow" data-customrow="${number}" data-customid="0">
            ...
            <p class="text-muted small tooltipText row"></p>
	    </div>
        `;
    ...
    $(document).on("change", ".typeSelect", function () {
        var $row = $(this).closest(".elementRow");
        var selectedType = $(this).val();
        var customIdOptions = $("#customIdAddBtn").data("values");

        var item = customIdOptions.find(x => x.Id == selectedType);

        if (item) {
            $row.find(".tooltipText").text(item.TooltipText);
        }
    });
    ...
    $("#elementsContainer").sortable({
        ...
        update: function (event, ui) {
            ...
            $(".elementRow").each(function (index) {
                $(this).attr("data-customrow", index + 1);
            });
        ...
        stop: function (event, ui) {
            ...
            // check if dropped outside container
            if (
                x < offset.left ||
                y < offset.top ||
                x > offset.left + width ||
                ...
                ui.item.remove(); // delete row
            }

            // update order numbers
            $("#elementsContainer .elementRow").each(function (index) {
                $(this).attr("data-customrow", index + 1);
            });
    ...
    $("#elementsContainer").disableSelection();
